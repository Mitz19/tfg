...
class WebotsROS2Node(Node):
    def __init__(self):
        ...
        self.help_active = False  # Indica si el robot está respondiendo a una llamada de ayuda
    ...
    def get_help_position(self):
        help = self.robot.getFromDef("_1")
        if help:
            return help.getPosition()
        return None

    def move_towards_help(self):
        help_position = self.get_help_position()
        if not help_position:
            print("Help not found.")
            self.linear_velocity = 0.0
            self.angular_velocity = 0.0
            return
        
        robot_position = self.robot.getSelf().getPosition()
        dx = help_position[0] - robot_position[0]
        dy = help_position[1] - robot_position[1]
        distance = math.sqrt(dx**2 + dy**2)

        # Si estamos cerca del help, detenernos
        if distance < 0.5:
            print("Reached help.")
            self.help_active = False  # Desactivar la bandera
            self.linear_velocity = 0.0
            self.angular_velocity = 0.0
            return

        # Calcular ángulo hacia el help
        target_angle = math.atan2(dy, dx)
        current_orientation = self.robot.getSelf().getOrientation()
        current_angle = math.atan2(current_orientation[1], current_orientation[0])
        angle_difference = target_angle - current_angle

        # Ajustar velocidades para movernos hacia el help
        self.linear_velocity = 5.0  # Velocidad constante hacia adelante
        self.angular_velocity = angle_difference * 2.0  # Ajuste del ángulo
    ...
    def command_callback(self, msg):
        ...
        if command != self.current_command:
            ...
            if command == "avanza":
                ...
                print("Moving towards help call.")
                self.help_active = True
                self.move_towards_help()
            else:
    ...
    def step(self):
        ...
        if detected:
            ...
            elif obj_type == "prohibited":
                self.print_message_once("Forbidden zone detected. Stopping immediately.")
                self.linear_velocity = 0.0
                self.angular_velocity = 0.0
                self.control_logic()
                if self.help_active:
                    self.print_message_once("Contradiction detected: Cannot enter prohibited zone but a human needs my help.")
                    rclpy.shutdown()
                    exit(0)
                self.current_command = None  # Limpiar comando actual
        else:
